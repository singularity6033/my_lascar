...
- all the statistical computations that you would like to process on them.
...
# First we need a Container, let's use BasicAesSimulationContainer. Say with 10000 traces, and the noise set to 1.
from Lib_SCA.lascar import SimulatedPowerTraceContainer

# container = BasicAesSimulationContainer(10000, noise=1)

container = SimulatedPowerTraceContainer(50)
...
- VarEngine: compute the variance of all the traces leakages
...
# session.output_method = MatPlotLibOutputMethod(session.engines["mean"])
session.run()
