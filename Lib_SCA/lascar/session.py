...
class Session:
    ...
    - distribute the batches to the registered engines. 'engines'
    ...
    def run(self, batch_size=100, thread_on_update=True):
        """
        Core function of Session: read all traces from the container, and distribute them to the engines, and manage results
        ...
        """
        if batch_size == "auto":
            self._batch_size = self._find_optimal_batch_size(self.container.leakages.dtype,
                                                             self.container.leakages.shape[1])
        else:
