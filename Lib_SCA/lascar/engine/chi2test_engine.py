...
class Chi2TestEngine(PartitionerEngine):
    ...
    def _initialize(self):
        ...
        self._histogram = np.zeros((self._partition_size,) + self._session.leakage_shape + (len(self._bin_starts),),
    ...
    def _finalize(self):
        ...
        for idx_sample in np.ndindex(self._session.leakage_shape):
            ...
            condition = [
                not all([self._histogram[idx_part][idx_sample][k] == 0 for idx_part in range(self._partition_size)]) for
                k in range(len(self._bin_starts))]
