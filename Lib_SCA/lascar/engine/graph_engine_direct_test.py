...
class GraphTestEngine(PartitionerEngine):
    ...
    def _finalize(self):
        ...
        threshold_r = np.mean(init_graph_r.w_to_unw_thresholds)
        ...
        threshold_f = np.mean(init_graph_f.w_to_unw_thresholds)

        # threshold used to convert weighted graph to unweighted one
        self.threshold = (threshold_r + threshold_f) / 2
        ...
        idx_r0, idx_r1 = graph_samples_r > self.threshold, graph_samples_r <= self.threshold
        graph_samples_r[idx_r0], graph_samples_r[idx_r1] = 0, 1
        idx_f0, idx_f1 = graph_samples_f > self.threshold, graph_samples_f <= self.threshold
...
class TraceBasedGraphTestEngine(PartitionerEngine):
    ...
    def _update(self, batch):
        ...
        self.threshold.append(self._calc_best_conversion_threshold(adj_matrix_r))
        self.threshold.append(self._calc_best_conversion_threshold(adj_matrix_f))
    ...
    def _finalize(self):
        ...
        self.threshold = np.mean(self.threshold)

        # convert to unweighted graphs
        idx_r0, idx_r1 = np.array(self._graph_set_r) > self.threshold, graph_samples_r <= self.threshold
        graph_samples_r[idx_r0], graph_samples_r[idx_r1] = 0, 1
        idx_f0, idx_f1 = graph_samples_f > self.threshold, graph_samples_f <= self.threshold
        graph_samples_f[idx_f0], graph_samples_f[idx_f1] = 0, 1
