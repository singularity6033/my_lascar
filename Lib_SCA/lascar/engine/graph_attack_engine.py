...
class GraphAttackEngineTraceAllCorr(A_GraphConstructionTraceAllCorr):
    ...
    def __init__(self,
        ...
        A_GraphConstructionTraceAllCorr.__init__(self, name, selection_function, guess_range, solution)
    ...
    def _finalize(self):
        for bit in tqdm(range(len(self._bits))):
            for guess in range(self._number_of_guesses):
                ...
                self.l2[bit, guess, 0], self.l2[bit, guess, 1] = self.l2[bit, guess, 0] / m_r, self.l2[bit, guess, 1] / m_f
                self.ll[bit, guess, 0], self.ll[bit, guess, 1] = self.ll[bit, guess, 0] / m_r, self.ll[bit, guess, 1] / m_f
                ...
                self.results[bit, guess] = self.sd.spectral_distance(graph_samples_r, graph_samples_f)
        return self.results
...
class GraphAttackEngineTraceAllDist(A_GraphConstructionTraceAllDist):
    ...
    def __init__(self,
        ...
        :param selection_function: takes a value and a guess_guess as input, returns 0 or 1
        :param guess_range: what are the values for the guess guess
        :param num_bins: used for cdf estimation in ks
        :param solution: if known, indicate the correct guess guess.
        :param k: k largest eigenvalues to be compared
        :param mode: 'l2', 'lmax', 'ks' --> Kolmogorov–Smirnov (K-S) distance/statistic
        """
        self.sd = SpectralDistance(mode, k)
        A_GraphConstructionTraceAllDist.__init__(self, name, selection_function, guess_range, num_bins, solution)
    ...
    def _finalize(self):
        ...
        for bit in tqdm(range(len(self._bits))):
            for guess in range(self._number_of_guesses):
                m_r, m_f = self._count[bit, guess, 0], self._count[bit, guess, 1]
                pdf_r, pdf_f = np.array(self.hist_counts[bit][guess][0]) / m_r, np.array(self.hist_counts[bit][guess][1]) / m_f
